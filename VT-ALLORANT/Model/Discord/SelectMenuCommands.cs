...
public class SelectMenuCommands()
{
    public static async Task RankSelectMenu(SocketMessageComponent component)
    {
        ...
        Player player = Player.Load(player => player.DiscordUser.DiscordId == component.User.Id);
    ...
    {
        string[] data = component.Data.CustomId.Split(":");
        Team joiningTeam = Team.Load(Player.Load(player => player.DiscordUser.DiscordId == component.User.Id));
